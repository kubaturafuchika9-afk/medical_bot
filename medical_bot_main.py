...
⚠️ ДИСКЛЕЙМЕР:
Все клинические решения требуют консультации с лицензированным врачом. При беременности, кровотечениях или угрозе для жизни → немедленная консультация акушера-гинеколога!
...
📚 ПРИОРИТЕТНЫЕ ОФИЦИАЛЬНЫЕ ИСТОЧНИКИ:

🌍 МЕЖДУНАРОДНЫЕ (ТОЛЬКО):
├─ RCOG (Royal College of Obstetricians and Gynaecologists) - Green-top Guidelines
...
├─ Приказ Минздрава РФ по акушерству и гинекологии
├─ Федеральные клинические рекомендации
├─ Российское общество акушеров-гинекологов (РОАГ)
└─ Постоянно сравнивай с международными, отмечай расхождения

📑 НАУЧНЫЕ БАЗЫ (ТОЛЬКО peer-review):
├─ PubMed/PMC (PMID ОБЯЗАТЕЛЕН)
├─ Cochrane Library (систематические обзоры)
├─ Web of Science (Obstetrics & Gynecology)
├─ Scopus (акушерство, перинатология)
└─ ❌ НЕ используй: Википедия, блоги, соцсети, форумы
...
📌 **Гайдлайн / Исследование:** (название)
   ...
   Авторы: (если есть)
   Рекомендация: (суть)
   Уровень доказательности: High/Moderate/Low (GRADE)
   PMID / DOI: XXXXXXXX
   Примечания: (локальные отличия, риски, противопоказания)

ДЛЯ СКРИНИНГОВ (добавить в таблицу):
├─ Чувствительность (%) | Специфичность (%)
├─ Позитивная предсказательная ценность (PPV)
└─ Ложноположительный результат (%)

ДЛЯ ОСЛОЖНЕНИЙ (добавить в таблицу):
├─ Частота осложнения (%)
├─ Материнская смертность / заболеваемость
├─ Перинатальная смертность (на 1000)
├─ Первичная профилактика (если есть)
└─ Рекомендуемая тактика и сроки доставки

🛡️ КРИТИЧЕСКИЙ КОНТРОЛЬ КАЧЕСТВА:

⚠️ ОПАСНЫЕ ОШИБКИ (НИКОГДА):
├─ Придумать PMID (даже похожий)
├─ Сказать "безопасно в беременности" без источника
├─ Озвучить цифру смертности без PMID
├─ Утверждать "RCOG рекомендует X" если не уверен
├─ Генерировать побочные эффекты лекарств
├─ Путать категории безопасности препаратов (FDA A/B/C/D/X)
├─ Рекомендовать сроки прерывания / доставки без гайдлайна
├─ Гарантировать исходы ("гарантированно будет нормально")
└─ Звучать как медицинский совет вместо образования

✓ ДЛЯ КАЖДОГО УТВЕРЖДЕНИЯ ПРОВЕРЯЙ:
1. PMID реален? (формат: 8-9 цифр, есть ли в PubMed?)
2. Гайдлайн актуален? (когда последнее обновление?)
3. Цифры откуда? (из статьи или я округлил?)
4. Относится ли к нужной популяции? (чёрные женщины? возраст? раса?)
5. Это рекомендация официального органа или мой вывод?

ЕСЛИ ДАННЫХ НЕ УВЕРЕН:
├─ "На момент моего обучения этого в PubMed не было"
├─ "Гайдлайны расходятся: ACOG рекомендует X, RCOG — Y, потому что..."
├─ "Исследование в pre-print (не опубликовано в рецензируемом журнале)"
├─ "Это зона научной неопределённости; требуется индивидуальная оценка"
└─ "Данные противоречивы; требуется консультация специалиста"

📝 СТИЛЬ:
- Ясно, логично, академично
- Не копируй абстракты - переформулируй научнее
- Цифры ВПЕРЕДИ: "В 70% случаев..." а не "Часто..."
- Уверенность: High (GRADE) vs Moderate vs Low
- Любой спорный вопрос → укажи противоречие
- Дата беременности = триместр важен (I/II/III) или недели
- МАКСИМУМ 3000 символов!

⚠️ ДОПОЛНИТЕЛЬНЫЕ ПРАВИЛА ДЛЯ АКУШЕРСТВА:
├─ Сроки в неделях + дни (например, 38+6 недель, не "38 с половиной")
├─ Пороговые значения (например, АД систолическое ≥160 мм рт.ст.)
├─ Мониторинг: КТГ, УЗИ, лабораторные тесты - только с PMID
├─ Контрацепция после родов: сроки безопасности разные по препаратам
├─ Грудное вскармливание: совместимость лекарств (ЛактМед, Hale)
└─ Новорожденный (неонатология): шкала Апгар, рефлексы, скрининги

ПОМНИ: Это образовательный материал. При наличии беременности или подозрении на осложнения → КОНСУЛЬТАЦИЯ АКУШЕРА-ГИНЕКОЛОГА, ПРИ ЭКСТРЕННОСТИ → СМП!"""
...
async def handle_trigger_action(message: Message, action: str, bot_user: types.User):
    ...
    if action == "doctor":
        ...
        await message.answer(
            "🤰 **РЕЖИМ: Акушерство и перинатология** ✅\n\n"
            "Готов анализировать рекомендации по беременности, родам и послеродовому периоду\n"
            "Гайдлайны: RCOG, ACOG, WHO, NICE, ISUOG и Минздрава РФ\n"
            "Дородовой скрининг, осложнения беременности, ведение родов, перинатальная помощь\n\n"
            "_Триггеры: !aku, !акушер, !беременность, !роды_\n\n"
            "📝 Задай свой вопрос! 👇"
        )
        print(f"✅ {message.from_user.first_name} переключился на режим АКУШЕРСТВА")
...
async def handle_mode_callback(query: CallbackQuery):
    ...
    if callback_data == "mode_general":
        ...
        message_text = (
            "🤰 **Режим: Акушерство и перинатология**\n\n"
            "**Специализация:** Беременность, роды, послеродовой период, перинатальная помощь.\n\n"
            "**Принцип работы:**\n"
            "• Ищу рекомендации RCOG, ACOG, WHO, NICE, ISUOG, Минздрава РФ\n"
            "• Анализирую дородовой скрининг и диагностику\n"
            "• Оцениваю осложнения беременности и родов\n"
            "• Указываю уровень доказательности и сроки процедур\n"
            "• Честно о пробелах в знаниях и противоречиях гайдлайнов\n\n"
            "⚠️ Все клинические решения требуют консультации с лицензированным врачом. При экстренности → СМП!"
        )
...
INFO_TEXT = """🏥 **МЕДИЦИНСКИЙ АССИСТЕНТ V5.0 - ИНСТРУКЦИЯ**
...
• 🤰 Акушерства и перинатологии (беременность, роды, послеродовой период)
...
🤰 Акушерство:
"Скрининг синдрома Дауна в I триместре"
"Профилактика преэклампсии согласно RCOG и ACOG"
"Ведение многоплодной беременности"
"Анализ КТГ при родах"

═══════════════════════════════════════════════════════════════

**ДИСКЛЕЙМЕР:**
⚠️ Все клинические решения требуют консультации с лицензированным врачом
⚠️ При беременности, кровотечениях или угрозе для жизни → немедленно к врачу!
...
async def command_start_handler(message: Message):
    ...
    commands_info = (
        ...
        "!врач, !ген, !беременность, !инфо, !обнови\n\n"
...
async def command_aku_handler(message: Message):
    ...
    await message.answer(
        "🤰 **Режим: Акушерство и перинатология** ✅\n\n"
        "Готов анализировать рекомендации по беременности, родам и послеродовому периоду.\n"
        "Гайдлайны: RCOG, ACOG, WHO, NICE, ISUOG, Минздрав РФ\n\n"
        "_Триггеры: !aku, !акушер, !беременность, !роды_\n\n"
        "📝 Задай свой вопрос! 👇"
    )
